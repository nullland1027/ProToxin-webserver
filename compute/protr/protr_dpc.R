...
for (i in seq_along(test_fasta)) {
  seq <- test_fasta[[i]]
  feature_list <- append(feature_list, list(extractDC(seq)))
...
# 筛选
selected_cols <- df[, c("SL", "CS", "QL", "CC", "VE", "HC", "CY", "VC", "RV", "GY", "EI", "GA", "LA", "ID", "VR", "DI", "AL", "QA", "CF", "SC", "CE")]

# 将数据框写入CSV文件
result_filename <- "neg_dpc_21_all.csv"
